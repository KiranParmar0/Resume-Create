...
async function sendEmailsInBatches(transporter, hrContacts) {
  ...
  for (let i = 0; i < hrContacts.length; i += CONFIG.batchSize) {
    ...
    console.log(`\n📧 Sending batch ${Math.floor(i / CONFIG.batchSize) + 1}/${Math.ceil(hrContacts.length / CONFIG.batchSize)} (${batch.length} emails)...\n`);
...
/**
 * Main function - orchestrates the entire email sending process
 */
async function main() {
  console.log("========================================");
